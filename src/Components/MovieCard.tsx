...
const MovieCard = ({ data }) => {
  ...
  const Field = ({ label, keyName, keyboardType = 'default' }) => (
    <View style={{ marginBottom: verticalScale(10) }}>
      <Text style={styles.inputHeading}>{label}</Text>
      <TextInput
  value={editedData[keyName]?.toString() || ''}
  onChangeText={(text) => setEditedData(prev => ({ ...prev, [keyName]: text }))}
...
/>
      ...
      Alert.alert(
        ...
        [
          ...
          },
          {
            ...
            style: 'destructive',
            onPress: async () => {
              const success = await deleteMovie(data.id);
              if (success) {
                setselected(false); 
              }
            },
  ...
  return (
    <View>
      ...
      <Modal animationType="slide" transparent={true} visible={selected} onRequestClose={() => setselected(false)}>
        <View style={styles.MainModalContainer}>
          <View style={styles.ModalContainer}>
            <TouchableOpacity onPress={() => setselected(false)} style={styles.CrossBtn}>
              <Image source={require('../assets/Icons/cross.png')} style={{ width: verticalScale(25), height: verticalScale(25) }} />
            </TouchableOpacity>

            {isAdmin ? (
              <ScrollView>
                <View style={styles.imageArea}>
                  <Image source={{ uri: editedData.poster || data.poster_url }} style={styles.poster} />
                  <TouchableOpacity style={styles.chooseBtn}><Text>Choose Poster</Text></TouchableOpacity>
                  <TouchableOpacity style={[styles.chooseBtn, { backgroundColor: '#fff' }]} onPress={handleSave}><Text>Save Data</Text></TouchableOpacity>
                </View>
              ...
              <ScrollView>
                ...
                <View style={styles.ModalDataCenter}>
                  <Text style={[styles.title, { color: '#000' }]}>{data.title} <Text style={styles.ModalSubTitle}>({data.release_year})</Text></Text>
                  <Text style={styles.ModalSubTitle2}>{data.genre}</Text>
                  <View style={styles.RatingSection}>
                    <Image source={require('../assets/Icons/star.png')} style={styles.RatingIcon} />
                    <Text style={[styles.ModalSubTitle2, { marginTop: verticalScale(-2) }]}>{data.rating}</Text>
                  </View>
                  <Text style={styles.ModalSubTitle}>Director: <Text style={styles.ModalSubTitle2}>{data.director}</Text></Text>
